\section{Example} \label{sect:example}
...
Let's calculate the critical: points, angles and distances; for a code 110 wheel profile with a $3\degree$ tread slope.

\paragraph{Given:}
\begin{itemize}
	\item A wheel profile where $code = 110$, and
	\item Wheel tread with $slope = \theta_s = 3\degree$
\end{itemize}	
...
\paragraph{Note:}
When using trigonometric functions you may need to convert angles from degrees to radians and back again.
This will depend upon your calculator, spreadsheet, math programming library, etc.
\begin{itemize}
	\item To convert from degrees to radians:
	\begin{equation*}
	\theta_{rads} = \theta_{deg} \cdot \left( \frac{\pi}{180\degree} \right)
	\end{equation*}
	\item To convert from radians to degrees: 
	\begin{equation*}
	\theta_{degs} = \theta_{rad} \cdot \left( \frac{180\degree}{\pi} \right)
	\end{equation*}
\end{itemize}
...
\item The value of the tread slope was given as:
\begin{equation*}
slope = \theta_s = 3\degree
\end{equation*}

\item Substitute values for $R1$ and $slope = \theta_s$ into equation \ref{eqn:x_s} to find $x_s$.
\begin{align*}
	x_s &= R1 \cdot \sin(\theta_s) \\
	x_s &= 0.014 \cdot \sin(3\degree) \\
	x_s &= 0.014 \cdot 0.0523359562 \\
	x_s &= 0.0007327034\ inch
\end{align*}		

\item Calculate $y_s$ by substituting values for $R1$ and $slope = \theta_s$ into equation \ref{eqn:y_s}.
\begin{align*}
	y_s &= R1 \cdot \cos(\theta_s) \\
	y_s &= 0.014 \cdot \cos(3\degree) \\
	y_s &= 0.014 \cdot 0.9986295348	\\
	y_s &= 0.0139808135\ inch
\end{align*}		

\item Now plug in the calculated value of $y_s$ and the published value for $P$ into equation \ref{eqn:y_g} to find $y_g$.
\begin{align*}
	y_g &= y_s - P \\
	y_g &= 0.0139808135 - 0.010 \\
	y_g &= 0.0039808135\ inch
\end{align*}		
...
\item Equation \ref{eqn:p_g} states gaging point $p_g$ will always be at the origin by definition.
\begin{equation*}
	p_g = (0, 0)
\end{equation*}
...
\begin{align*}
	d' &= P - y_2 + y_d \\
	d' &= 0.010 - 0.0051181888 + 0.0178579377 \\
	d' &= 0.022739748 \\
	d' &= 0.0227\ inch
\end{align*}
