...
\begin{itemize}
	...
	\item Then mark the calculated locations for points: $p_g$, $p_s$, $p_1$, $p_2$, $p_3$ and $p_d$.
...
For example $\theta_g$ for $Code\ 110$ at $\theta_s = 3\degree$ is $16.5196$, whereas it is $-0.0785$ at $\theta_s = 3\degree$ for $Code\ 93$.

In Table \ref{table:mytable} you will notice that changing the slope angle $\theta_s$ within any particular wheel code has slight effect on $\theta_g$.
Furthermore, changing $\theta_s$ has negligible effect upon point locations.
Table \ref{table:mytable} is deliberately redundant to show that, when rounded to four decimal places, point locations are effectively independent of tread slope.

As a final thought, the location of $p_d$ is at flange depth $d'$, which is well above the flange depth $D'$ given in the RP-25 document.
