...
\begin{itemize}
	\item First draw $x$ and $y$ axes at the desired origin.
...
Drawings for adjacent wheel codes will have different relative spacing between points.
...
Also, if you need point locations for an intermediate value of slope angle, say for $\theta_s = 1.5\degree$, you should perform calculations with equations \ref{eqn:x_s} through \ref{eqn:d_prime}.

Furthermore, you can propose wheel profiles for new wheel codes that follow the RP-25 form.
To do this, propose new values for $N'$, $T$, $W$, $D'$, $P$, $R1$, $R2$ and $R3$.  
Then declare the code number of the new profile based upon the value of $N'$.
Next use equations \ref{eqn:x_s} through \ref{eqn:d_prime} to determine the proposed profile: distances, angles and points.
