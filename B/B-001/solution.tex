...
The wheel profile can be drawn either:
\begin{itemize}
	\item Manually, with paper, pencil and drafting tools; or,
	\item Electronically, with a computer aided drafting (CAD) program.
\end{itemize}

If drawing manually, you will want to scale up all dimensions by a ratio of $20:1$ or so.
Also when drawing manually, a dial caliper or machinist's ruler, graduated in tenths and hundredths of an inch, will be needed.
...
To the left of the intersection, the sloped line becomes the wheel tread.

Mark the point where the horizontal line and wheel tread line intersect.
Label the point $p_s$.
Point $p_s$ is the point where the wheel fillet will end and the wheel tread begins. 
...
Furthermore, from Theorem 2 we know that line through circle center $p_1$ and point $p_s$ is perpendicular to a tangent line.
...
The arc should pass thru point $p_s$ and intersect the new horizontal line.
...
Let's apply our ``Trick'' from Section \ref{sec_strategy}.
...
\begin{enumerate}
	...
	insertion point, then drag and snap $p_g$ to the coordinate system origin.  Verify all: points, lines, arcs 
	and labels; follow $p_g$ and maintain their relative positions.
\end{enumerate}
...
The RP-25 document says the circles of radius $R1$ and $R2$ are tangent to each other at the gaging point $p_g$.
...
%%%%%
\subsection{Step 6: Drawing circle of radius $R2$}

\begin{figure}
	\centering
	\input{python/draw/output/fig6.tikz}
	\caption{\\Step 6: Drawing circle of radius $R2$}
	\label{fig:fig_f6}
\end{figure}
...
Observe that the arcs of radius $R2$ and $R3$ form a tangent ogive
\endnote{
...
Note: $p_d$ is at some distance $d'$ (not $D'$) below $p_s$.
...
Label the drawing with: $D'$, $N'$, $P$ and $W$; add the vertical and horizontal 
...
Since point $p_g$ is a distance $P$ below point $p_s$, it can be seen that $y_s = y_g + P$ and: 
...
Point $p_d$ is the bottom of the flange where the flange halves intersect.
Let's label the lengths of the horizontal and vertical sides $x_d$ and $y_d$ respectively.

The hypotenuse is of length $R2$.

From figures \ref{fig:fig_f10} and \ref{fig:fig_f11c} we can see the horizontal distance $x_2$, found in equation \ref{eqn:x_2}, is just a distance $x_d$ to the right of $T/2$.
...
Let's begin by choosing the gaging point $p_g$ to be the origin, such that $p_g$ will always be at coordinate (0, 0).
\begin{equation}
	\label{eqn:p_g}
	\boxed{
		p_g = (0, 0)
...
Keep in mind the $D'$ is the maximum flange depth allowed by RP-25.
