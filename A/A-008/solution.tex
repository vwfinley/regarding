...
Since line $L_s$ is a given from the problem statement, and from equation \ref{eq:abd} its angle $\alpha$ with the $x$-axis, can either: be calculated from its slope, or measured directly.  
...
Use the cosine function to find $x_A$.
\begin{equation} \label{eq:eq_xa}
x_A = \overline{BD} = -r \cdot \cos{\alpha}   
\end{equation}

Since $r$ was found in equation \ref{eq:eq_r}, use equations \ref{eq:eqi} and \ref{eq:eq_xa} to find $A$.
\begin{align}
A &= (x_A, y_A)\\
A &= (\overline{BD}, -i)\\
