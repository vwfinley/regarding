...
The location of point $A$, the circle center can be calculated. Using the cosine function to find $A_x = \overline{BD}$.
Now that $r$ has been found, use equation \ref{eq:eqi} to find $-i$.
\begin{align*}
A &= (x_A, y_A)\\
A &= (\overline{BD}, -i)\\
...
Point $C$ is then easily found since $h$ was given in the problem statement and $x_A$ has already been calculated:
\begin{align}
C &= (r \cdot \cos{\alpha}, h)
\end{align}

Point $D$ also uses the value of $x_A = x_D$ 
\begin{equation}
D = (r \cdot \cos{\alpha}, 0) 
\end{equation}
