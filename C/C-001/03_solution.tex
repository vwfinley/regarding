...
Our development environment will roughly look like Figure 1.

\begin{verbatim}
Laptop (Windows)   Remote Dev machine (Linux)   Golang Dev Docker Container
----------       ----------      ------------
| VSCode |------>|Docker  |----->| Golang   |
|        |       | Git    |      |          |
----------       ---|------      ------------
                    |
                    V
...
Another popluar solution is to run the Linux instance in a Cloud VM service.
...
Linux distributions fall into two main familes that differ by package manager and available packages.
...
Better IDEs probably exist, but I am familar with VSCode.
...
For performace reasons it is worth noting the laptop configuration.
Knowing the laptop configuration helps when setting expectations and making comparisions to other hardware environments.
